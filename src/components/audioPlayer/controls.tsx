...
const Controls: FC<ControlsProps> = ({ isPlaying, setIsPlaying, handleNext, handlePrev }) => {
  return (
    <IconContext.Provider value={{ size: "35px", color: "var(--theme-second-color)" }}>
      <S.ControlsWrapper>
        ...
        <S.PlayPauseBtn onClick={() => setIsPlaying(!isPlaying)}>{isPlaying ? <FaPause /> : <IoPlay />}</S.PlayPauseBtn>
